...
const routeProvider = function (app) {
...
router.use('{{ channelName | toHermesTopic }}', async (message, next) => {
  try {
    ...
    {% endfor -%}
    if (nValidated === 1) {
      await {{ channelName | camelCase }}Handler.{{ channel.publish().id() }}({app, message});
      next()
    } else {
...
router.useOutbound('{{ channelName | toHermesTopic }}', async (message, next) => {
  try {
    ...
    {% endfor -%}
    if (nValidated === 1) {
      await {{ channelName | camelCase }}Handler.{{ channel.subscribe().id() }}({app, message});
      next()
    } else {
